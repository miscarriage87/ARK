...
export default async function ArchivePage({ params, searchParams }: { params: Promise<{ username: string }>, searchParams: Promise<{ filter?: string }> }) {
    ...
    const user = await prisma.user.findUnique({
        ...
        include: {
            views: {
                where: {
                    date: { lte: today }  // Nur vergangene/heutige Zitate, keine vorgenerierten
                },
                include: { quote: true },
                orderBy: { date: 'desc' }  // Nach Zitat-Datum sortieren
            },
            ratings: {
                include: { quote: true },
    ...
    let quotes: (Quote & { displayDate: string })[] = [];
    const showingFavorites = filter === 'favorites';

    if (showingFavorites) {
        // Favoriten: Rating-Datum als displayDate (YYYY-MM-DD Format)
        quotes = user.ratings.map(r => ({
            ...r.quote,
            displayDate: r.createdAt.toISOString().split("T")[0]
        }));
    } else {
        // Default: History - das eigentliche Zitat-Datum verwenden
        quotes = user.views.map(v => ({ ...v.quote, displayDate: v.date }));
    }
                    ...
                    {quotes.map((quote, i) => (
                        <div key={i} style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', opacity: 0.6 }}>
                                ...
                                <span>{new Date(quote.displayDate + 'T00:00:00').toLocaleDateString('de-DE')}</span>
                            </div>
                            {quote.explanation && (
